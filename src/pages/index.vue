...
const connectionMeta = computed(() => {
  switch (store.connection.status) {
    case 'connected':
      return { label: '已连接', className: 'surface-success', icon: Wifi }
    case 'connecting':
      return { label: '连接中', className: 'surface-info', icon: Cable }
    case 'reconnecting':
      return { label: '重连中', className: 'surface-warning', icon: RefreshCw }
    case 'error':
      return { label: '连接错误', className: 'surface-destructive', icon: WifiOff }
    case 'disconnected':
      return { label: '未连接', className: 'text-muted-foreground', icon: Unplug }
    default:
      return { label: '未配置', className: 'text-muted-foreground', icon: Settings }
  }
})
...
function eventClass(event: EventLogEntry): string {
  if (event.level === 'error')
    return 'surface-destructive border-l-destructive'

  if (event.level === 'warning')
    return 'surface-warning border-l-warning'

  return 'border-border border-l-primary/50 bg-secondary/40'
}
...
<template>
  <TooltipProvider>
    <main class="grid-surface min-h-screen bg-background">
      <div class="mx-auto flex w-full max-w-7xl flex-col gap-5 px-4 py-5 sm:px-6 lg:px-8">
        <header class="panel flex flex-col gap-4 px-4 py-4 lg:flex-row lg:items-center lg:justify-between">
          <div class="min-w-0 space-y-2.5">
            <div class="flex flex-wrap items-center gap-2">
              ...
              <Badge v-if="store.status?.network.wifiSignal" variant="outline" class="readout text-muted-foreground">
                {{ store.status.network.wifiSignal }}
              </Badge>
            </div>
            <div>
              <h1 class="flex items-center gap-2.5 text-2xl font-semibold tracking-tight sm:text-3xl">
                <span class="inline-block h-6 w-1 rounded-full bg-primary sm:h-7" />
                拓竹打印监控台
              </h1>
              <p class="readout mt-1.5 truncate text-sm text-muted-foreground">
          ...
          <div class="flex flex-wrap items-center gap-2">
            <Tooltip>
              <TooltipTrigger as-child aria-label="切换主题">
                <Button type="button" variant="outline" size="icon" aria-label="切换主题" @click="toggleDark()">
        ...
        <Alert v-if="notice" class="surface-success">
        ...
        <section class="space-y-4">
          ...
          <template v-else>
            <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <Card class="relative overflow-hidden border-l-2 border-l-primary/70">
                <CardHeader class="pb-2">
                  <CardTitle class="flex items-center gap-2 text-xs font-medium uppercase tracking-wider text-muted-foreground">
                    <Activity class="size-4" />
                    打印状态
                  </CardTitle>
                </CardHeader>
                <CardContent class="space-y-2">
                  <div class="readout text-2xl font-semibold">
              ...
              <Card class="relative overflow-hidden border-l-2 border-l-warning/70">
                <CardHeader class="pb-2">
                  <CardTitle class="flex items-center gap-2 text-xs font-medium uppercase tracking-wider text-muted-foreground">
                    <Gauge class="size-4" />
                    进度
                ...
                <CardContent class="space-y-3">
                  <div class="readout text-2xl font-semibold">
                    {{ store.status?.state.progress ?? 0 }}<span class="text-lg text-muted-foreground">%</span>
                  </div>
                  <Progress :model-value="store.status?.state.progress ?? 0" />
                </CardContent>
              </Card>

              <Card class="relative overflow-hidden border-l-2 border-l-destructive/60">
                <CardHeader class="pb-2">
                  <CardTitle class="flex items-center gap-2 text-xs font-medium uppercase tracking-wider text-muted-foreground">
                ...
                <CardContent class="space-y-1 text-sm">
                  <div class="flex justify-between gap-3">
                    ...
                    <span class="readout font-medium">{{ store.status ? formatTemperature(store.status.temperatures.nozzle) : '--' }}</span>
                  </div>
                  <div class="flex justify-between gap-3">
                    <span class="text-muted-foreground">热床</span>
                    <span class="readout font-medium">{{ store.status ? formatTemperature(store.status.temperatures.bed) : '--' }}</span>
                  </div>
                </CardContent>
              </Card>

              <Card class="relative overflow-hidden border-l-2 border-l-success/60">
                <CardHeader class="pb-2">
                  <CardTitle class="flex items-center gap-2 text-xs font-medium uppercase tracking-wider text-muted-foreground">
            ...
            <Card>
              <CardHeader>
                <CardTitle class="flex items-center gap-2 text-sm font-medium uppercase tracking-wider">
              ...
              <CardContent class="space-y-4">
                ...
                <div class="grid gap-2 sm:grid-cols-4">
                  <Button
                    ...
                    :class="String(option.level) === store.status?.state.speedLevel ? 'border-primary bg-primary/10 text-primary' : ''"
            ...
            <Tabs v-if="store.hasStatus" default-value="overview" class="w-full">
                ...
                <Card>
                  ...
                  <CardContent class="space-y-4 text-sm">
                    <div class="grid grid-cols-2 gap-3">
                      <div class="panel bg-secondary/40 p-3">
                        <p class="text-xs uppercase tracking-wider text-muted-foreground">
                          层数
                        </p>
                        <p class="readout mt-1 font-medium">
                          {{ formatLayer(store.status?.state.currentLayer, store.status?.state.totalLayers) }}
                        </p>
                      </div>
                      <div class="panel bg-secondary/40 p-3">
                        <p class="text-xs uppercase tracking-wider text-muted-foreground">
                        ...
                        <p class="readout mt-1 font-medium">
                          {{ formatMinutes(store.status?.state.remainingMinutes) }}
                        </p>
                      </div>
                      <div class="panel bg-secondary/40 p-3">
                        <p class="text-xs uppercase tracking-wider text-muted-foreground">
                      ...
                      <div class="panel bg-secondary/40 p-3">
                        ...
                        <p class="readout mt-1 font-medium">
                          {{ formatDateTime(store.status?.jobTiming.estimatedDoneAt) }}
                        </p>
                      </div>
                    </div>
                    <Separator />
                    <div class="flex items-center justify-between gap-3">
                      ...
                      <span class="readout font-medium">{{ formatDateTime(store.connection.lastReportAt) }}</span>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent class="space-y-4 text-sm">
                    <div class="flex items-center justify-between">
                      ...
                      <span class="readout font-medium">{{ formatNumber(store.status?.temperatures.chamber) }} °C</span>
                    </div>
                    <Separator />
                    <div class="grid gap-3 sm:grid-cols-2">
                      <div v-for="fan in store.status?.fans ?? []" :key="fan.key" class="panel bg-secondary/40 p-3">
                        <p class="text-xs uppercase tracking-wider text-muted-foreground">
                          {{ fan.label }}
                        </p>
                        <p class="readout mt-1 text-lg font-semibold">
                          {{ formatFan(fan) }}
                        </p>
                      </div>
                    </div>
              ...
              <TabsContent value="video" class="mt-4">
                <Card>
                  ...
                  <CardContent>
                    <div class="aspect-video overflow-hidden rounded-md border-2 border-border/70 bg-slate-950">
                      <img
              ...
              <TabsContent value="trend" class="mt-4">
                <Card>
                  ...
                  <CardContent class="space-y-4">
                    <div class="grid-surface h-56 rounded-md border-2 border-border/70 bg-secondary/30 p-3">
                      <svg viewBox="0 0 100 48" preserveAspectRatio="none" class="h-full w-full">
                        ...
                        <polyline v-if="progressPolyline" :points="progressPolyline" fill="none" stroke="var(--chart-1)" stroke-width="1.8" vector-effect="non-scaling-stroke" />
                        <polyline v-if="nozzlePolyline" :points="nozzlePolyline" fill="none" stroke="var(--chart-2)" stroke-width="1.4" vector-effect="non-scaling-stroke" />
                        <polyline v-if="bedPolyline" :points="bedPolyline" fill="none" stroke="var(--chart-3)" stroke-width="1.4" vector-effect="non-scaling-stroke" />
                      </svg>
                    </div>
                    <div class="grid gap-2 text-sm sm:grid-cols-3">
                      <div class="flex items-center gap-2">
                        <span class="size-3 rounded-full" style="background: var(--chart-1)" />进度
                      </div>
                      <div class="flex items-center gap-2">
                        <span class="size-3 rounded-full" style="background: var(--chart-2)" />喷嘴
                      </div>
                      <div class="flex items-center gap-2">
                        <span class="size-3 rounded-full" style="background: var(--chart-3)" />热床
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <div class="grid gap-4 xl:grid-cols-2">
                  <Card v-for="unit in store.status?.ams.units ?? []" :key="unit.id">
                    <CardHeader>
                      ...
                      <CardDescription class="readout">
                        湿度 {{ formatNumber(unit.humidity) }} · 温度 {{ formatNumber(unit.temperature) }} °C
                      </CardDescription>
                    </CardHeader>
                    <CardContent class="grid gap-3 sm:grid-cols-2">
                      <div v-for="tray in unit.trays" :key="tray.id" class="rounded-md border p-3 transition-colors" :class="tray.active ? 'surface-info' : 'bg-secondary/40'">
                        <div class="flex items-start justify-between gap-3">
                          ...
                          <span class="size-6 shrink-0 rounded-full border-2 border-border" :style="{ backgroundColor: trayColor(tray) }" />
                        </div>
                        <div class="readout mt-3 grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                  ...
                  <Card v-if="store.status?.ams.externalSpool">
                    ...
                    <CardContent>
                      <div class="rounded-md border bg-secondary/40 p-3">
                        <div class="flex items-center justify-between gap-3">
                          <div>
                            <p class="font-medium">
                              {{ store.status.ams.externalSpool.label }}
                            </p>
                            <p class="text-sm text-muted-foreground">
                              {{ store.status.ams.externalSpool.type ?? '--' }}
                            </p>
                          </div>
                          <span class="size-6 rounded-full border-2 border-border" :style="{ backgroundColor: trayColor(store.status.ams.externalSpool) }" />
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                ...
                <div class="grid gap-4 lg:grid-cols-2">
                  <Card>
                    ...
                    <CardContent class="space-y-3">
                      ...
                      <Alert v-for="message in store.status?.hms ?? []" :key="message.code" :variant="hmsVariant(message)">
                        <AlertTriangle class="size-4" />
                        <AlertTitle class="readout">{{ message.code }}</AlertTitle>
                  ...
                  <Card>
                    ...
                    <CardContent class="space-y-3">
                      ...
                      <div v-for="event in recentEvents" :key="event.id" class="rounded-md border border-l-2 p-3 text-sm" :class="eventClass(event)">
                        <div class="flex items-start justify-between gap-3">
                          <p class="font-medium">
                            {{ event.message }}
                          </p>
                        ...
                        <p class="readout mt-1 text-xs opacity-75">
                          {{ formatDateTime(event.at) }} · {{ event.kind }}
                        </p>
                      </div>
                    </CardContent>
                  </Card>
